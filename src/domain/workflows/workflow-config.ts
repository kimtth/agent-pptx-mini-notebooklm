...
export const WORKFLOW_CONFIGS: Record<WorkflowId, WorkflowConfig> = {
  ...
  poststaging: {
    ...
    steps: [
      ...
      'If blocking issues exist (missing images, icon loss at 40%+ missing ratio, ERROR-level overlap/overflow), repair the generated preview artifacts and rerun the render.',
      'If only actionable contrast issues remain, patch the relevant generated JSON inputs and rerun.',
      'If all findings are clear, confirm the deck is ready.',
    ],
    agentDirective: 'Use this workflow for post-generation QA and user-driven visual fixes. Inspect the QA report (if provided), summarize findings, and either confirm success or use patch_layout_infrastructure + rerun_pptx tools to fix issues. For visual changes (font, color, fill, style), read and patch render_config. For content/layout changes, patch layout_input or layout_specs. Do not inspect application source files — target only the generated preview artifacts.',
